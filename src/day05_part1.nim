...
proc solve(input: string): int =
  ...
  echo fmt"boundary_ids: {boundary_ids}"

  # Check each id against the fresh slices.
  for id in numbers:
    let insert_position = upperBound(boundary_ids, id)
    let is_edge = insert_position > 0 and boundary_ids[insert_position - 1] == id
    let is_fresh = is_edge or insert_position mod 2 == 1
    echo fmt"id: {id}, insert_position: {insert_position}, is_edge: {is_edge}, is_fresh: {is_fresh}"
    if is_fresh:
      result += 1
  return result
